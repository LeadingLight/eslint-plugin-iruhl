...
module.exports = {
  ...
  create: function(context) {
    ...
    const actionsLast = configuration.actionsLast || false;
    ...
    function isActionPropName(propName) {
      return /^do[A-Z]/.test(propName);
    }
    ...
    function checkSorted(declarations) {
      ...
      declarations.reduce((prev, curr, idx, decls) => {
        ...
        const previousIsAction = isActionPropName(prevPropName);
        const currentIsAction = isActionPropName(currentPropName);
        ...
        if (actionsLast && !callbacksLast) {

          if (!previousIsAction && currentIsAction) {
            // Entering the action prop section
            return curr;
          }
          if (previousIsAction && !currentIsAction) {
            // Encountered a non-action prop after a action prop
            context.report({
              node: prev,
              message: 'Action prop types must be listed after all other prop types'
            });
            return prev;
          }
        }
        ...
        if (actionsLast && callbacksLast) {
          if (!(previousIsAction || previousIsCallback) && currentIsAction) {
            // Entering the action prop section
            return curr;
          }
          if (previousIsAction && !(currentIsAction || currentIsCallback)) {
            // Encountered a non-action prop after a action prop
            context.report({
              node: prev,
              message: 'Action prop types must be listed after all regular prop types'
            });
            return prev;
          }
        }
